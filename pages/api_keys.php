...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .quick-jump { display:flex; gap:8px; flex-wrap:wrap; margin:0 0 16px; }
        ...
        .key-tools { display:flex; gap:8px; align-items:center; margin-bottom:10px; }
        .key-search { flex:1; min-width:0; background:var(--surface); border:1px solid var(--border); color:var(--text); border-radius:8px; padding:8px 10px; font-family:'DM Mono',monospace; font-size:12px; outline:none; }
        .key-search:focus { border-color:var(--accent); }
        .key-meta-summary { font-size:11px; color:var(--text-muted); }
...
<nav>
    ...
    <div class="nav-links">
        <a href="/pages/api_docs.php" class="nav-link">📄 Docs</a>
...
<div class="container">
    ...
    <div class="quick-jump" aria-label="API key sections">
        <a href="#planCard">Plan</a>
        <a href="#docsBanner">Docs</a>
        <a href="#keysSection">Your Keys</a>
    </div>
...
function formatClientDate(input, options = { year: 'numeric', month: 'short', day: 'numeric' }) {
    const parsed = parseServerDate(input);
    return parsed ? parsed.toLocaleString(undefined, options) : 'Unknown';
}
...
function renderApp(username, plan) {
    ...
    document.getElementById('app').innerHTML = `
        <div class="plan-card" id="planCard">
            ...
            <a href="/pages/pricing.php" class="btn btn-outline" style="font-size:11px;padding:6px 12px">Upgrade</a>
        </div>

        <div class="docs-banner" id="docsBanner">
            <p><strong>New to the API?</strong> Check out the documentation to learn how to authenticate and make requests.</p>
            <a href="/pages/api_docs.php" class="btn btn-outline" style="font-size:11px;padding:6px 12px;white-space:nowrap">View Docs →</a>
        </div>

        <div class="section-title" id="keysSection">
            <span>Your API Keys</span>
            <button class="btn btn-primary" style="font-size:11px;padding:6px 14px" onclick="toggleNewKeyForm()">+ New Key</button>
        </div>

        <div class="key-tools">
            <input class="key-search" id="keySearch" type="text" placeholder="Filter keys by label or key..." oninput="renderKeys()">
            <div class="key-meta-summary" id="keyMetaSummary">Showing 0 keys</div>
        </div>
...
async function loadKeys() {
    ...
    keyRows = Array.isArray(data.keys) ? data.keys : [];
    renderKeys();
}
...
function renderKeys() {
    ...
    if (!container) return;
    const q = String(document.getElementById('keySearch')?.value || '').trim().toLowerCase();
    const rows = q
        ? keyRows.filter(k => [k.label, k.api_key].map(v => String(v || '').toLowerCase()).join(' ').includes(q))
        : keyRows;

    const meta = document.getElementById('keyMetaSummary');
    if (meta) meta.textContent = `Showing ${rows.length} of ${keyRows.length} keys`;

    if (!rows.length) {
        container.innerHTML = keyRows.length
            ? '<div style="text-align:center;padding:30px;color:var(--text-muted);font-size:13px">No keys matched your filter</div>'
            : '<div style="text-align:center;padding:30px;color:var(--text-muted);font-size:13px">No API keys yet — generate one above</div>';
        return;
    }
    ...
    container.innerHTML = rows.map(k => {
        ...
        const created = formatClientDate(k.created_at, { year: 'numeric', month: 'short', day: 'numeric' });
        const lastUsed = k.last_used_at ? formatClientDate(k.last_used_at, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Never';
        return `
        <div class="key-card">
            <div class="key-top">
                <span class="key-label">${escHtml(k.label)}</span>
                <span class="${k.active == 1 ? 'key-active' : 'key-disabled'}">${k.active == 1 ? 'Active' : 'Disabled'}</span>
                <span style="font-size:11px;color:var(--text-muted);margin-left:auto">Created ${created}</span>
            </div>
