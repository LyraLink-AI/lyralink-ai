...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .ev-ai_policy_block,.ev-ai_public_policy_block,.ev-ai_image_block{background:rgba(245,158,11,0.12);color:var(--warn);border:1px solid rgba(245,158,11,0.28)}
        .ev-ai_rate_limited{background:rgba(59,130,246,0.12);color:#93c5fd;border:1px solid rgba(59,130,246,0.28)}
...
<div class="page">
    ...
    <div class="stats" id="statsRow">
        ...
        <div class="stat"><div class="stat-label">AI Policy Blocks (24h)</div><div class="stat-val" id="s-ai-policy" style="color:var(--warn)">—</div></div>
    ...
    <div class="toolbar">
        ...
        <button class="filter-btn" onclick="setFilter('ai_policy_block',this)">AI Blocks</button>
        <button class="filter-btn" onclick="setFilter('ai_image_block',this)">Image Blocks</button>
        <button class="filter-btn" onclick="setFilter('ai_rate_limited',this)">AI Rate Limits</button>
...
async function load() {
    ...
    // Stats
    const now24 = events.filter(e => {
        const ts = parseServerDate(e.created_at);
        return ts && ts.getTime() > Date.now() - 86400000;
    });
    ...
    document.getElementById('s-ai-policy').textContent = data.stats?.ai_policy_blocks_24h ?? now24.filter(e => e.event_type === 'ai_policy_block' || e.event_type === 'ai_public_policy_block').length;
    ...
    const failRecent = now24.filter(e => {
        const ts = parseServerDate(e.created_at);
        return e.event_type === 'login_fail' && ts && ts.getTime() > cutoff;
    });
    const ipCounts = {};
