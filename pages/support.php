...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .quick-jump{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 14px}
        ...
        @media(max-width:640px){
            ...
            .quick-jump{flex-wrap:nowrap;overflow-x:auto;padding-bottom:2px}
            .quick-jump button{white-space:nowrap}
        }
...
<div class="container">
    ...
    <div id="mainView" class="screen active">
        <div style="margin-bottom:20px">
            ...
            <p style="font-size:12px;color:var(--text-muted)">Get help with your deployment, billing, or account</p>
        </div>

        <div class="quick-jump" aria-label="Support quick actions">
            <button type="button" onclick="switchTab('new')">New Ticket</button>
            <button type="button" onclick="switchTab('mine')">My Tickets</button>
            <button type="button" onclick="scrollToReplyBox()">Jump to Reply</button>
        </div>
        <div class="quick-hint">Shortcuts: 1 new ticket, 2 my tickets, / focus current search/input</div>

        <div class="tabs">
            <div class="tab active" data-tab="new" onclick="switchTab('new',this)">🎫 New Ticket</div>
            <div class="tab" data-tab="mine" onclick="switchTab('mine',this)">📋 My Tickets</div>
        </div>
            ...
            <div class="form-grid">
                <div class="form-field">
                    ...
                    <select id="ticketCat">
                        ...
                        <option value="live_chat">Live Chat Integration</option>
                    </select>
                </div>
                <div class="form-field">
...
let lastLiveReplyCount = -1;
let supportSocket = null;
let supportSocketTicketRef = null;
let supportSocketReconnect = null;
let supportSocketManualClose = false;
...
async function loadMyTickets() {
    ...
    if (!currentRef) {
        const active = data.tickets.find(t => !['resolved','closed'].includes(String(t.status || '')));
        if (active) {
            await openTicket(active.ticket_ref);
        }
    }
}
...
async function openTicket(ref) {
    ...
    lastLiveReplyCount = Array.isArray(data.replies) ? data.replies.length : 0;
    ...
    connectSupportSocketForCurrentTicket();
}

function isSupportSocketOpen() {
    return supportSocket && supportSocket.readyState === WebSocket.OPEN;
}

function closeSupportSocket(manual = true) {
    if (supportSocketReconnect) {
        clearTimeout(supportSocketReconnect);
        supportSocketReconnect = null;
    }
    supportSocketManualClose = manual;
    if (supportSocket) {
        try { supportSocket.close(); } catch (_) {}
        supportSocket = null;
    }
    if (manual) {
        supportSocketTicketRef = null;
    }
}

function supportSocketUrl(url, token) {
    const sep = url.includes('?') ? '&' : '?';
    return `${url}${sep}token=${encodeURIComponent(token)}`;
}

function scheduleSupportSocketReconnect() {
    if (!currentRef || supportSocketReconnect) return;
    supportSocketReconnect = setTimeout(() => {
        supportSocketReconnect = null;
        connectSupportSocketForCurrentTicket(true);
    }, 3000);
}

async function connectSupportSocketForCurrentTicket(force = false) {
    if (!currentRef) return;
    if (!force && supportSocketTicketRef === currentRef && isSupportSocketOpen()) return;

    closeSupportSocket(true);
    const body = currentEmail ? { ref: currentRef, email: currentEmail } : { ref: currentRef };
    const auth = await api('ws_auth_user', body).catch(() => null);
    if (!auth?.success || !auth.url || !auth.token) {
        return;
    }

    supportSocketTicketRef = currentRef;
    supportSocketManualClose = false;
    try {
        supportSocket = new WebSocket(supportSocketUrl(auth.url, auth.token));
    } catch (_) {
        scheduleSupportSocketReconnect();
        return;
    }

    supportSocket.onmessage = async (event) => {
        let payload = null;
        try {
        ...
        if (!payload.ticket_ref || payload.ticket_ref !== currentRef) return;
        if (!shouldRefreshLiveTicket()) return;
        await openTicket(currentRef);
    };

    supportSocket.onclose = () => {
        const shouldReconnect = !supportSocketManualClose && supportSocketTicketRef === currentRef;
        supportSocket = null;
        if (shouldReconnect) {
            scheduleSupportSocketReconnect();
        }
...
async function loadLiveTicket(autoOpen = true) {
    const body = currentEmail ? { email: currentEmail } : {};
    const data = await api('live_ticket', body).catch(() => null);
    if (!data?.success || !data.ticket) return null;

    if (autoOpen) {
        if (activeTab !== 'mine') {
            const mineTab = Array.from(document.querySelectorAll('.tab')).find(el => /my tickets/i.test(el.textContent || ''));
            if (mineTab) switchTab('mine', mineTab);
        }
        if (!currentRef || currentRef !== data.ticket.ticket_ref) {
            await openTicket(data.ticket.ticket_ref);
        }
    }
...
function shouldRefreshLiveTicket() {
    if (!currentRef) return false;
    const replyBox = document.getElementById('replyMsg');
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '/') {
        e.preventDefault();
        if (activeTab === 'mine' && document.getElementById('lookupEmail')) {
