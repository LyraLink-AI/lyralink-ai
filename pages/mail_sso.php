...
if ($crossHost) {
    if ($cookieDomain === '') {
        sso_fail('Cross-host webmail requires MAIL_WEBMAIL_COOKIE_DOMAIN (example: .lyralinkai.com)');
    }
...
if ($redirectHost === '' || $redirectHost !== $webmailHost) {
    $redirectUrl = rtrim($webmailOrigin, '/') . '/?_task=mail';
}
