...
<head>
    ...
    <link rel="canonical" href="https://status.lyralinkai.com/">
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .live-badge{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:999px;border:1px solid rgba(34,197,94,0.28);background:rgba(34,197,94,0.08);color:#86efac;font-size:10px;font-weight:700;letter-spacing:.3px}
        .live-dot{width:7px;height:7px;border-radius:50%;background:var(--op);box-shadow:0 0 0 0 rgba(34,197,94,.5);animation:livePulse 1.8s infinite}
        @keyframes livePulse{0%{box-shadow:0 0 0 0 rgba(34,197,94,.45)}70%{box-shadow:0 0 0 7px transparent}100%{box-shadow:0 0 0 0 transparent}}
        ...
        .status-jump{display:flex;gap:8px;flex-wrap:wrap;margin:-16px 0 28px}
        .status-jump a{font-size:11px;color:var(--text-muted);text-decoration:none;padding:6px 12px;border:1px solid var(--border);border-radius:999px;background:rgba(124,58,237,0.06);transition:all .2s}
        .status-jump a:hover{border-color:var(--accent);color:var(--accent-light);background:rgba(124,58,237,0.12)}
        .status-chip{font-size:11px;color:var(--text-muted);padding:6px 12px;border:1px solid var(--border);border-radius:999px;background:rgba(17,17,24,0.85)}
        .trust-links{display:flex;gap:8px;flex-wrap:wrap;margin:-4px 0 20px}
        .trust-link{font-size:11px;color:var(--text-muted);text-decoration:none;padding:6px 12px;border:1px solid var(--border);border-radius:999px;background:rgba(124,58,237,0.06);transition:all .2s}
        ...
        .service-row{display:flex;align-items:center;gap:14px;padding:14px 18px;border-bottom:1px solid rgba(30,30,46,0.5);transition:background 0.15s;min-width:0;overflow:hidden}
        .service-row:last-child{border-bottom:none}
        .service-row:hover{background:rgba(124,58,237,0.03)}
        .service-meta{display:flex;flex-direction:column;gap:4px;flex:0 0 180px;min-width:0}
        .service-name{font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .service-desc{font-size:10px;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .service-right{display:flex;align-items:center;gap:10px;flex:1;min-width:0}
        .service-uptime-bar{display:flex;gap:2px;align-items:center;flex:1;min-width:0;overflow:hidden}
        ...
        /* UPTIME CONTROLS */
        .uptime-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:10px}
        .range-switcher{display:inline-flex;align-items:center;gap:6px;padding:4px;border:1px solid var(--border);border-radius:999px;background:rgba(17,17,24,0.9)}
        .range-btn{border:none;background:transparent;color:var(--text-muted);padding:7px 12px;border-radius:999px;font-size:10px;font-family:'DM Mono',monospace;cursor:pointer;transition:all .2s}
        .range-btn:hover{color:var(--accent-light)}
        .range-btn.active{background:rgba(124,58,237,0.18);color:var(--accent-light);box-shadow:0 0 0 1px rgba(124,58,237,0.24) inset}
        .range-note{font-size:10px;color:var(--text-muted)}
        ...
        @media (max-width: 640px) {
            ...
            .status-jump { margin:-10px 0 20px; flex-wrap:nowrap; overflow-x:auto; padding-bottom:2px; }
            .status-jump a, .status-chip { white-space:nowrap; }

            .section-label { font-size: 9px; }
            .group-header { padding: 8px 12px; font-size: 9px; }
            .uptime-toolbar { align-items:flex-start; }
            .range-switcher { width:100%; justify-content:space-between; }
            .range-btn { flex:1; padding:7px 0; font-size:9px; }

            /* Stack service rows on small screens */
            .service-row { padding: 10px 12px; gap: 8px; align-items:flex-start; }
            .service-meta { flex: 0 0 94px; }
            .service-name { font-size: 11px; }
            .service-desc { font-size: 9px; }
            .service-right { gap: 6px; }
...
<div class="page">
    ...
    <div class="status-jump reveal" aria-label="Status quick navigation">
        <a href="#servicesSection">Services</a>
        <a href="#incidentsSection">Active Incidents</a>
        <a href="#historySection">History</a>
        <span class="status-chip">Services: <span id="quickServiceCount">0</span></span>
        <span class="status-chip">Active: <span id="quickIncidentCount">0</span></span>
    </div>
    <div class="trust-links reveal" aria-label="Trust center links">
        <a class="trust-link active" href="/pages/status">Status</a>
    ...
    <div class="section reveal" id="servicesSection">
        ...
        <div class="uptime-toolbar">
            <div class="range-switcher" id="rangeSwitcher">
                <button class="range-btn" data-range="10" onclick="setUptimeRange(10)">10 days</button>
                <button class="range-btn" data-range="30" onclick="setUptimeRange(30)">30 days</button>
                <button class="range-btn" data-range="60" onclick="setUptimeRange(60)">60 days</button>
                <button class="range-btn active" data-range="90" onclick="setUptimeRange(90)">90 days</button>
            </div>
            <div class="range-note" id="rangeNote">Showing the last 90 days</div>
        </div>
        ...
        <div class="uptime-legend">
            <span id="legendStartLabel">90 days ago</span>
...
let selectedUptimeRange = 90;

async function loadStatus() {
    const url = '/api/status.php?action=get_status&t=' + Date.now();
    ...
    currentStatusPayload = data;
    updateStatusQuickSummary(data);
    updateBanner(data.overall);
    renderServices(data.services, data.uptime, selectedUptimeRange);
    renderIncidents(data.incidents);
    renderHistory(data.resolved);

    const d = new Date(data.generated);
    document.getElementById('lastUpdated').textContent = 'Updated ' + d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
}

function updateStatusQuickSummary(data) {
    const serviceCount = Array.isArray(data?.services) ? data.services.length : 0;
    const incidentCount = Array.isArray(data?.incidents) ? data.incidents.length : 0;
    const svc = document.getElementById('quickServiceCount');
    const inc = document.getElementById('quickIncidentCount');
    if (svc) svc.textContent = String(serviceCount);
    if (inc) inc.textContent = String(incidentCount);
}
...
function setUptimeRange(days) {
    selectedUptimeRange = [10, 30, 60, 90].includes(Number(days)) ? Number(days) : 90;
    document.querySelectorAll('.range-btn').forEach(btn => {
        btn.classList.toggle('active', Number(btn.dataset.range) === selectedUptimeRange);
    });
    const note = document.getElementById('rangeNote');
    if (note) note.textContent = `Showing the last ${selectedUptimeRange} days`;
    const legendStart = document.getElementById('legendStartLabel');
    if (legendStart) legendStart.textContent = `${selectedUptimeRange} days ago`;
    if (currentStatusPayload) {
        renderServices(currentStatusPayload.services || [], currentStatusPayload.uptime || {}, selectedUptimeRange);
    }
}

function renderServices(services, uptime, rangeDays = 90) {
    // Group by category
    const groups = {};
    services.forEach(s => {
    ...
    const safeRange = [10, 30, 60, 90].includes(Number(rangeDays)) ? Number(rangeDays) : 90;
    const days  = [];
    ...
    for (const [cat, svcs] of Object.entries(groups)) {
        ...
        svcs.forEach(s => {
            ...
            const bars = days.map(d => {
                const rawPct = svcUptime[d] ?? null;
                const pct = rawPct === null ? 100 : Number(rawPct);
                totalDays++;
                uptimeSum += pct;
                let cls = 'uptime-day up', tip = '';
                if (rawPct === null)     { cls = 'uptime-day up';       tip = d + ': 100%'; }
            ...
            html += `<div class="service-row">
                <div class="service-meta">
                    ...
                    <span class="service-desc">${escHtml(s.description || 'Live service status and uptime')}</span>
                </div>
