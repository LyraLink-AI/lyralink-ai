...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .quick-jump { display:flex; gap:8px; flex-wrap:wrap; margin:0 0 12px; }
        .quick-jump button { border:1px solid var(--border); background:rgba(124,58,237,0.08); color:var(--text-muted); border-radius:999px; padding:6px 12px; font-size:11px; font-family:'DM Mono',monospace; cursor:pointer; transition:all .2s; }
        .quick-jump button:hover { border-color:var(--accent); color:var(--accent-light); background:rgba(124,58,237,0.14); }
        .quick-hint { font-size:10px; color:var(--text-muted); margin:0 0 12px; }
...
<div class="container">
    ...
    <div class="quick-jump" aria-label="Dataset manager sections">
        <button type="button" onclick="switchTab('pending')">Pending</button>
        <button type="button" onclick="switchTab('dataset')">Dataset</button>
        <button type="button" onclick="switchTab('search')">Test Search</button>
        <button type="button" onclick="switchTab('hf')">Hugging Face</button>
    </div>
    <div class="quick-hint">Shortcuts: 1 Pending, 2 Dataset, 3 Search, 4 Hugging Face, / focus active search field</div>
    ...
    <!-- TABS -->
    <div class="tabs">
        <div class="tab active" data-tab="pending" onclick="switchTab('pending', this)">Pending</div>
        <div class="tab" data-tab="dataset" onclick="switchTab('dataset', this)">Dataset</div>
        <div class="tab" data-tab="search" onclick="switchTab('search', this)">Test Search</div>
        <div class="tab" data-tab="hf" onclick="switchTab('hf', this)">Hugging Face</div>
    </div>
    ...
    <div id="tab-pending">
        ...
        <div class="table-wrap">
            <table>
                <thead><tr><th>ID</th><th>User</th><th>IP Address</th><th>Question</th><th>Answer</th><th>Date</th><th>Actions</th></tr></thead>
                <tbody id="pendingBody"><tr><td colspan="7" class="empty">Loading...</td></tr></tbody>
            </table>
        </div>
    ...
    <!-- HUGGING FACE TAB -->
    <div id="tab-hf" style="display:none">
        <div class="toolbar">
            <input class="search-box" type="text" id="hfQuery" placeholder="Search Hugging Face datasets (e.g. gsm8k, squad, openwebtext)..." style="flex:1">
            <button class="btn btn-primary" onclick="searchHfDatasets()">Search HF</button>
            <button class="btn btn-success" onclick="delegateBestHfDataset()" id="hfDelegateBtn">Auto Pick + Train</button>
        </div>
...
function switchTab(tab, el) {
    activeTab = tab;
    ['pending','dataset','search','hf'].forEach(t => {
        document.getElementById('tab-' + t).style.display = 'none';
    });
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    if (!el) {
        el = document.querySelector(`.tab[data-tab="${tab}"]`);
    }
...
async function loadPending(page = 1) {
    ...
    if (!data.rows?.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="empty">No pending conversations 🎉</td></tr>';
    ...
    tbody.innerHTML = data.rows.map(r => `
        <tr>
            ...
            <td style="color:var(--accent-light);font-size:11px">${escHtml(r.user_id)}</td>            <td style="color:var(--text-muted);font-size:11px;white-space:nowrap;font-family:'DM Mono',monospace">${escHtml(r.ip_address || '—')}</td>            <td class="td-q">${escHtml(r.user_message)}</td>
            <td class="td-a">${escHtml(r.ai_reply)}</td>
            <td style="color:var(--text-muted);font-size:11px;white-space:nowrap">${formatClientDate(r.created_at, true)}</td>
...
async function searchHfDatasets() {
    ...
    if (!query) {
        showToast('Type a Hugging Face query first', 'warn');
        return;
    }

    const resultsEl = document.getElementById('hfResults');
    resultsEl.innerHTML = '<div style="color:var(--text-muted);font-size:12px">Searching Hugging Face...</div>';

    const data = await api('GET', 'hf_search', { query, limit: 12 });
    if (!data.success) {
        resultsEl.innerHTML = '<div class="empty">Failed to fetch Hugging Face datasets</div>';
        showToast(data.error || 'Hugging Face search failed', 'error');
        return;
    }
    if (!data.rows?.length) {
        resultsEl.innerHTML = '<div class="empty">No Hugging Face datasets found for that query</div>';
        return;
    }

    resultsEl.innerHTML = data.rows.map(row => {
        const repoId = String(row.repo_id || '');
        const safeRepoId = escHtml(repoId);
        const clickRepoId = repoId.replace(/'/g, "\\'");
        const tags = Array.isArray(row.tags) ? row.tags.slice(0, 8) : [];
        const tagsHtml = tags.length
            ? `<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">${tags.map(tag => `<span class="badge-method">${escHtml(String(tag))}</span>`).join('')}</div>`
            : '';

        return `
            <div class="search-result">
                <div class="search-result-score">
                    <span class="badge-method">huggingface</span>
                    ${typeof row.score === 'number' ? `Score: ${Number(row.score).toFixed(3)} &nbsp;·&nbsp; ` : ''}
                    ${row.downloads != null ? `Downloads: ${Number(row.downloads).toLocaleString()} &nbsp;·&nbsp; ` : ''}
                    ${row.likes != null ? `Likes: ${Number(row.likes).toLocaleString()} &nbsp;·&nbsp; ` : ''}
                    ${row.last_modified ? `Updated: ${escHtml(row.last_modified)}` : 'Updated: —'}
                </div>
                <div class="search-result-q">${safeRepoId}</div>
                <div class="search-result-a">${escHtml(row.description || 'No description provided.')}</div>
                ${tagsHtml}
                <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
                    <a class="btn btn-outline" style="padding:6px 10px;text-decoration:none" href="${escHtml(row.url || ('https://huggingface.co/datasets/' + repoId))}" target="_blank" rel="noopener">Open on Hugging Face</a>
                    <button class="btn btn-success" style="padding:6px 10px" onclick="importHfDataset('${clickRepoId}')">Import to Dataset</button>
                </div>
            </div>
        `;
    }).join('');
...
async function importHfDataset(repoId) {
    const cleanRepoId = String(repoId || '').trim();
    if (!cleanRepoId) {
        showToast('Missing repo id', 'error');
        return;
    }

    const data = await api('POST', 'hf_import', { repo_id: cleanRepoId });
    if (!data.success) {
    ...
    if (data.imported === false) {
        showToast(`Already imported: ${cleanRepoId}`, 'warn');
    } else {
        showToast(`Imported ${cleanRepoId} into dataset`, 'success');
    }

    const downloadedCount = Array.isArray(data.download?.downloaded) ? data.download.downloaded.length : 0;
    const ingestedCount = Number(data.ingest?.inserted || 0);
    const authState = data.auth?.token_configured ? 'token-auth on' : 'public mode';
    if (downloadedCount > 0) {
        showToast(`Downloaded ${downloadedCount} file(s) locally (${authState})`, 'success');
    } else if (data.download?.attempted) {
        showToast(`No files downloaded (${authState})`, 'warn');
    }
    if (ingestedCount > 0) {
        showToast(`Ingested ${ingestedCount} training samples`, 'success');
    }
...
async function delegateBestHfDataset() {
    const query = document.getElementById('hfQuery').value.trim();
    if (!query) {
        showToast('Type what you want to train for first', 'warn');
        return;
    }

    const btn = document.getElementById('hfDelegateBtn');
    const resultsEl = document.getElementById('hfResults');
    btn.disabled = true;
    btn.textContent = 'Selecting + Importing...';
    resultsEl.innerHTML = '<div style="color:var(--text-muted);font-size:12px">Delegating best dataset selection and importing into local dataset...</div>';

    const data = await api('POST', 'hf_delegate_train', { query, train_now: 0, prepare_corpus: 0, ingest_max_rows: 200, download_files: 0, prefer_rows_api: 1, limit: 12 });

    btn.disabled = false;
    btn.textContent = 'Auto Pick + Import';

    if (!data.success) {
        resultsEl.innerHTML = '<div class="empty">Could not delegate dataset selection</div>';
        showToast(data.error || 'Delegate import failed', 'error');
        return;
    }

    const selected = data.selected || {};
    const selectedRepo = String(selected.repo_id || data.import?.repo_id || '');
    const trainOk = data.training?.attempted ? !!data.training?.ok : null;
    const trainLabel = trainOk === null ? 'not requested' : (trainOk ? 'ok' : 'failed');
    const downloadedCount = Array.isArray(data.download?.downloaded) ? data.download.downloaded.length : 0;
    const ingestedCount = Number(data.ingest?.inserted || 0);
    const storageDir = String(data.download?.storage_dir || '');
    const authState = data.auth?.token_configured ? 'token-auth enabled' : 'public-only mode';

    resultsEl.innerHTML = `
        <div class="search-result">
            <div class="search-result-score">
                <span class="badge-method">delegated</span>
                ${typeof selected.score === 'number' ? `Rank score: ${Number(selected.score).toFixed(3)} &nbsp;·&nbsp; ` : ''}
                Training: ${escHtml(trainLabel)} &nbsp;·&nbsp; Downloads: ${downloadedCount} &nbsp;·&nbsp; Ingested: ${ingestedCount} &nbsp;·&nbsp; ${escHtml(authState)}
            </div>
            <div class="search-result-q">Selected dataset: ${escHtml(selectedRepo || 'unknown')}</div>
            <div class="search-result-a">${escHtml(selected.description || 'Imported selected dataset into local memory and triggered continuous model learning.')}</div>
            ${storageDir ? `<div style="margin-top:8px;color:var(--text-muted);font-size:11px">Local dataset cache: ${escHtml(storageDir)}</div>` : ''}
            <div style="margin-top:8px;color:var(--text-muted);font-size:11px;white-space:pre-wrap">${escHtml(String(data.training?.output || '').slice(0, 1200) || 'No training output')}</div>
        </div>
    `;

    loadStats();
    showToast(`Selected ${selectedRepo || 'dataset'} and imported rows`, trainOk === false ? 'warn' : 'success');
}
...
async function api(method, action, params = {}) {
    try {
        ...
        if (!rawText) {
            return { success: false, error: `Request returned an empty response (HTTP ${response.status}).` };
        }

        try {
            const parsed = JSON.parse(rawText);
            return parsed && typeof parsed === 'object' ? parsed : { success: false, error: 'Invalid JSON response' };
        } catch (parseError) {
            return { success: false, error: `Invalid JSON response (${response.status}): ${parseError.message}` };
        }
    } catch (e) {
...
function formatClientDate(input, isDateOnly = false) {
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '1') { e.preventDefault(); switchTab('pending'); return; }
    if (e.key === '2') { e.preventDefault(); switchTab('dataset'); return; }
    if (e.key === '3') { e.preventDefault(); switchTab('search'); return; }
    if (e.key === '4') { e.preventDefault(); switchTab('hf'); return; }
    ...
    if (e.key === '/') {
        ...
        } else if (activeTab === 'hf') {
            document.getElementById('hfQuery')?.focus();
        }
    }
});
