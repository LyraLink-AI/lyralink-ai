...
function chat_select_capability_model(string $intent, string $plan, array $requiredCapabilities = [], array $routerMap = [], array &$routeMeta = []): string {
    ...
    $defaultModel = trim((string)($routerMap['default'] ?? 'lyralink-auto-canary:latest')) ?: 'lyralink-auto-canary:latest';
    $fallbackModel = trim((string)($routerMap['fallback'] ?? 'lyralink-fast:latest')) ?: 'lyralink-fast:latest';
...
function chat_request_trust_profile(string $latestUserMsg, bool $taskMode, string $taskFocus, array $taskControl = []): array {
    ...
    $toolPattern = '/\b(run|execute|scan|inspect|open|list\s+\/|list files|check logs|query\s+(my|the)\s+(server|db|database)|ssh|terminal|command output|check my production db)\b/i';
...
function chat_execution_records_from_legacy(array $legacy, string $requestId = ''): array {
    ...
    return [chat_execution_record(
        ...
        $status,
        [
            'authorization_state' => !empty($legacy['tool_authorized']) ? 'AUTHORIZED' : 'UNKNOWN',
...
function chat_tool_execution_state_from_context(array $requestProfile, array $context = []): array {
    ...
    $state['tool_required'] = $toolRequiredByClass || $toolRequiredByProfile;
    ...
    $sourceClass = in_array($requestClass, ['RESEARCH', 'SOURCE_REQUIRED'], true);
    if ($sourceClass) {
...
function chat_build_tool_execution_state(string $latestUserMsg, array $requestProfile, array $context = []): array {
    ...
    $toolRequired = (bool)($requestProfile['tool_required'] ?? false) || in_array($requestClass, ['SOURCE_REQUIRED', 'RESEARCH'], true);
...
function chat_answerability_arbitrator(string $latestUserMsg, string $reply, array $context = []): array {
    ...
    if (preg_match('/\b(outage|incident|502|browser-only|browser only|partial sql migration|intermittent failure)\b/i', $lower) === 1 && preg_match('/\b(no logs?|no traces?|no metrics?|not provided|not including logs|not including traces|not including metrics)\b/i', $lower) === 1) {
        return [
    ...
    if ($missingEvidence && preg_match('/\b(what caused|why failed|diagnose|analyze|root cause)\b/i', $lower) === 1) {
        return [
            'mode' => 'BOUNDED_ANALYSIS',
            'answerable' => true,
            'certainty_level' => 'limited',
...
function chat_trust_safe_fallback(string $latestUserMsg, array $answerability = [], array $issues = []): string {
    ...
    if (preg_match('/\b(latest server logs show|no logs|no command results are included)\b/i', $lower) === 1) {
    ...
    if ($mode === 'TOOL_LIMITATION') {
        return "This result is currently unverified because required tool access is unavailable in this runtime. Best bounded answer: use a read-only verification pass first, then run only reversible actions. Next checks: share command output, file listings, or logs so the conclusion can be verified against evidence.";
    }

    if ($mode === 'INSUFFICIENT_EVIDENCE') {
        return "This result is currently unverified because the required evidence or source is missing. Best bounded answer: separate confirmed facts from assumptions and avoid final claims until evidence is provided. Next checks: share the missing artifact, citation, logs, or command output to complete verification.";
    }
...
function chat_failure_regeneration_instruction(string $failureClass): string {
    ...
    return match ($mode) {
        ...
        'EVIDENCE_REQUIRED' => 'Regenerate with source honesty: request missing citation/source explicitly and avoid fabricating details.',
        'PRODUCTION_SAFETY' => 'Regenerate with conservative production safety: stabilize, preserve evidence, assess blast radius, then suggest reversible mitigations.',
...
function chat_self_verify_summary(string $latestUserMsg, string $reply, bool $taskMode, string $taskFocus, array $context = []): array {
    ...
    if (!empty($context['webSearchResults']) || !empty($researchCapability['enabled']) || !empty($researchCapability['available'])) {
    ...
    $outageWithMissingEvidence = preg_match('/\b(outage|incident|what caused)\b/i', $latestUserMsg) === 1
        && preg_match('/\b(no logs?|no traces?|no metrics?|not including logs|not including traces|not including metrics|not provided)\b/i', $latestUserMsg) === 1;
