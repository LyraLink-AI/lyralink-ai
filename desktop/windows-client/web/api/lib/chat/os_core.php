...
if (!function_exists('chat_os_route_classify')) {
    function chat_os_route_classify(array $requestProfile, array $taskControl, string $latestUserMsg, array $context = []): string {
        ...
        if (in_array($requestClass, ['RESEARCH', 'SOURCE_REQUIRED'], true) || !empty($context['needs_fresh_web'])) {
            return 'RESEARCH';
        }
        if (!empty($taskControl['requires_execution']) || in_array($requestClass, ['TOOL_REQUIRED', 'TOOL_UNAVAILABLE', 'SYSTEM_ADMINISTRATION', 'PRODUCTION_OPERATIONS'], true)) {
            return 'EXECUTION';
        }
...
if (!function_exists('chat_os_plan_actions')) {
    function chat_os_plan_actions(string $taskId, string $routeClass, string $capabilityId, array $resource, array $authorization, array $context = []): array {
        ...
        if ($routeClass === 'RESEARCH') {
            $steps = ['Generate query', 'Search sources', 'Retrieve candidates', 'Verify sources', 'Extract evidence', 'Draft response'];
        } elseif ($routeClass === 'EXECUTION') {
            $steps = ['Inspect request', 'Check capability', 'Check resource', 'Check authorization', 'Assess risk', 'Execute if allowed', 'Verify result'];
        } elseif ($routeClass === 'FAST') {
            $steps = ['Interpret request', 'Generate concise answer', 'Validate scope'];
        } else {
            $steps = ['Interpret request', 'Plan answer', 'Generate response', 'Validate response'];
        }
...
if (!function_exists('chat_os_prepare_runtime_decision')) {
    function chat_os_prepare_runtime_decision(string $latestUserMsg, array $requestProfile, array $taskControl, array $context = []): array {
        ...
        $researchRequested = in_array(strtoupper((string)($requestProfile['request_class'] ?? '')), ['RESEARCH', 'SOURCE_REQUIRED'], true)
