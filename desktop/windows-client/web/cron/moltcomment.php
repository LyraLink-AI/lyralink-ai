...
function callLlmHttp($url, $apiKey, $model, $messages, $maxTokens, $temperature) {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['model'=>$model,'messages'=>$messages,'max_tokens'=>$maxTokens,'temperature'=>$temperature]));
...
// Default LLM is Hermes via OpenRouter (LLM_PROVIDER); falls back to Groq on failure.
function callGroq($apiKey, $messages, $maxTokens = 150, $temperature = 0.85) {
    $provider = strtolower(trim(api_get_secret('LLM_PROVIDER', 'groq')));
    $orKey = api_get_secret('OPENROUTER_API_KEY', '');
    $oaKey = api_get_secret('OPENAI_API_KEY', '');

    if ($provider === 'openrouter' && $orKey !== '') {
        $model = trim(api_get_secret('OPENROUTER_MODEL', 'nousresearch/hermes-3-llama-3.1-70b')) ?: 'nousresearch/hermes-3-llama-3.1-70b';
        $reply = callLlmHttp('https://openrouter.ai/api/v1/chat/completions', $orKey, $model, $messages, $maxTokens, $temperature);
        if ($reply !== null) return $reply;
    } elseif ($provider === 'openai' && $oaKey !== '') {
        $model = trim(api_get_secret('OPENAI_MODEL', 'gpt-4o-mini')) ?: 'gpt-4o-mini';
        $base = rtrim(api_get_secret('OPENAI_BASE_URL', 'https://api.openai.com/v1'), '/');
        $reply = callLlmHttp("$base/chat/completions", $oaKey, $model, $messages, $maxTokens, $temperature);
        if ($reply !== null) return $reply;
    }

    return callLlmHttp('https://api.groq.com/openai/v1/chat/completions', $apiKey, 'llama-3.1-8b-instant', $messages, $maxTokens, $temperature);
}
