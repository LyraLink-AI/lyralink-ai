...
function askLyralink(prompt) {
    return new Promise((resolve) => {
        const config  = vscode.workspace.getConfiguration('lyralink');
        const apiKey  = config.get('apiKey');
        const apiUrl  = config.get('apiUrl') || 'https://lyralinkai.com/api/public_api.php';
        ...
        try {
            ...
            vscode.window.showErrorMessage('Lyralink: Invalid API URL in settings.');
            resolve(null);
            return;
        }

        const isLocalHttp = ['localhost', '127.0.0.1', '::1'].includes(urlObj.hostname);
        if (urlObj.protocol !== 'https:' && !(urlObj.protocol === 'http:' && isLocalHttp)) {
            vscode.window.showErrorMessage('Lyralink: API URL must use HTTPS (HTTP allowed only for localhost).');
            resolve(null);
            return;
        }

        if (urlObj.searchParams.has('key')) {
            vscode.window.showErrorMessage('Lyralink: Remove API key from URL query params and use configured API key header only.');
            resolve(null);
            return;
        }
