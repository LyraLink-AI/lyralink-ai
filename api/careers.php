...
// ── PUBLIC: APPLY ──
if ($action === 'apply') {
    $jobId      = (int)($_POST['job_id'] ?? $_POST['app_id'] ?? $_POST['app'] ?? $_POST['id'] ?? $_POST['job'] ?? 0);
    ...
    if (!$job) { echo json_encode(['success'=>false,'error'=>'Unknown job. Please refresh and pick a listed role.']); exit; }
