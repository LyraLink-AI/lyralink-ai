...
$streamStrictNoFallback = $streamResponseRequested
    && api_get_secret('CHAT_STREAM_STRICT_NO_FALLBACK', '1') === '1';
...
if ($streamUltraFastMode) {
    // Prioritize first-token latency for SSE by skipping heavy side paths.
...
$requestTrustProfile['intent'] = (string)($osRuntimeDecision['intent'] ?? ($requestTrustProfile['request_class'] ?? 'GENERAL_INFORMATION'));
$requestTrustProfile['desired_outcome'] = (string)($osRuntimeDecision['desired_outcome'] ?? 'ANSWER');
$requestTrustProfile['external_state_required'] = (bool)($osRuntimeDecision['external_state_required'] ?? false);
$requestTrustProfile['execution_required'] = (bool)($osRuntimeDecision['execution_required'] ?? false);
$requestTrustProfile['research_required'] = (bool)($osRuntimeDecision['research_required'] ?? false);
...
$requestTrustProfile['evidence_required'] = (bool)($osRuntimeDecision['evidence_required'] ?? false);
...
$forensicPre = [
    'request_id' => $traceId,
    'intent' => (string)($osRuntimeDecision['intent'] ?? ($requestTrustProfile['request_class'] ?? 'GENERAL_INFORMATION')),
    'desired_outcome' => (string)($osRuntimeDecision['desired_outcome'] ?? 'ANSWER'),
    'external_state_required' => (bool)($osRuntimeDecision['external_state_required'] ?? false),
    'execution_required' => (bool)($osRuntimeDecision['execution_required'] ?? false),
    'research_required' => (bool)($osRuntimeDecision['research_required'] ?? false),
    'evidence_required' => (bool)($osRuntimeDecision['evidence_required'] ?? false),
    'capabilities_required' => array_values((array)($osRuntimeDecision['capabilities_required'] ?? [])),
    'resources_required' => array_values((array)($osRuntimeDecision['resources_required'] ?? [])),
    'authorization_state' => (string)($osRuntimeDecision['authorization']['state'] ?? 'UNKNOWN'),
    'risk_level' => (string)($osRuntimeDecision['risk_level'] ?? 'LOW'),
    'routing_class' => (string)($osRuntimeDecision['route_class'] ?? 'STANDARD'),
...
trace_add($trace, $liveTrace, 'forensic', 'AI-OS forensic pre-execution snapshot', $forensicPre);
...
$skipExpensiveContext = (!$isSourceRequired && !$needsFreshWeb)
    && !$osRuntimeDecision['research']['requested']
...
if ($allowWebSearch && !$isImageRequest && $webSearchLifecycle['tool_required']) {
...
if (
    ...
    if (strtolower((string)$userModel) !== strtolower($forcedFastModel)) {
        trace_add($trace, $liveTrace, 'routing', ($force3bNonDeep ? 'Forced' : 'Auto-picked') . ' non-deep local model to fast tier', [
...
if (
    $streamUltraFastMode
    && !$modelExplicitlyRequested
    && in_array(strtolower($userProvider), ['local', 'hermes'], true)
    && !($attachmentMeta && (($attachmentMeta['type'] ?? '') === 'image'))
) {
    $ultraFastModel = llm_safe_local_model(
        (string)api_get_secret('CHAT_STREAM_FAST_MODEL', (string)($routerMap['fallback'] ?? 'lyralink-fast:latest')),
        (string)($routerMap['fallback'] ?? 'lyralink-fast:latest')
    );
    $userProvider = 'local';
    $userModel = $ultraFastModel;
    $routeMeta['stream_ultra_fast'] = true;
}
...
if (in_array(strtolower($userProvider), ['local', 'hermes'], true) && !llm_is_lyralink_model((string)$userModel)) {
    $previousModel = (string)$userModel;
    $userProvider = 'local';
    $userModel = llm_first_valid_model_for_plan('local', $userPlan);
    trace_add($trace, $liveTrace, 'routing', 'Non-LyraLink local model blocked; switched to LyraLink-safe model', [
        'previous_model' => $previousModel,
        'safe_model' => $userModel,
    ]);
}
...
if ($benchmarkMode && empty($webSearchResults)) {
    // Benchmark traffic must exercise the same truthfulness and safety contract
    // as ordinary traffic.  Previously this assignment discarded the earlier
    // guardrails entirely, which made incident and evidence-sensitive answers
    // materially less reliable than their production counterparts.
    $systemPrompt = 'You are Lyralink. Answer the user directly, accurately, and concisely. Follow every explicit scope, exclusion, count, and format constraint in the current request. When the user redirects away from an earlier topic, silently stay on the new task; do not repeat excluded prior-topic words. Never claim a tool, database, shell command, scan, API request, web search, source retrieval, or observation occurred unless the runtime provides verified evidence. Preserve unknown state and do not invent operational facts.';
    $systemPrompt .= "\n\n" . chat_reasoning_guardrails_prompt();
    $systemPrompt .= "\n\nBenchmark response contract:\n"
        . "- For a false premise, explicitly say the premise is false or incorrect, state the correction, and explain it without endorsing the false claim.\n"
        . "- For arithmetic or logic, show the decisive calculation or inference and state the final result. Distinguish relative percent changes from percentage points.\n"
        . "- For a request requiring a source, citation, current fact, or unavailable tool result, say plainly what cannot be verified or accessed; do not fabricate a source, result, or execution. Then give useful bounded next checks.\n"
        . "- For security, name the primary risk and give the shortest safe mitigation before secondary advice.\n"
        . "- For a production incident, use this order: stop changes or stabilize; preserve logs, traces, and metrics; assess blast radius and current state; choose reversible mitigation; validate data integrity and service recovery; then investigate root cause. Do not recommend broad dependency upgrades or blind rollback during the incident.\n"
        . "- For requested writing, return the requested number of polished options and retain the requested tone and purpose.";
...
if ($isUltraSimplePromptTurn && !$benchmarkMode) {
...
if ($isFastWritingTurn && !$benchmarkMode && empty($webSearchResults) && empty($datasetMatches)) {
...
if ($streamUltraFastMode) {
    $replyMaxTokens = min($replyMaxTokens, max(96, (int)api_get_secret('CHAT_STREAM_FAST_MAX_TOKENS', '220')));
    $preferFullReplies = false;
}
...
if ($streamUltraFastMode) {
    $replyTemperature = min($replyTemperature, 0.66);
}
...
if (!$reply && !$streamStrictNoFallback && !($attachmentMeta && ($attachmentMeta['type'] ?? '') === 'image')) {
    $lastUserOnly = '';
    for ($i = count($messages) - 1; $i >= 0; $i--) {
        if (($messages[$i]['role'] ?? '') === 'user') {
    ...
    if ($benchmarkMode) {
        ...
        if (!is_string($benchmarkFallback) || trim($benchmarkFallback) === '') {
            ...
            if ($sourceRequiredClass) {
                $benchmarkFallback = "Known: this request requires source-backed verification and a primary citation was not established in this run.\n"
                    . "Unknown: exact source authenticity, completeness, and whether the claim is reproduced in the primary source.\n"
                    . "Next checks:\n"
                    . "1. Provide the exact source URL/title or artifact identifier.\n"
                    . "2. Provide the specific claimed quote or data point.\n"
                    . "3. I will verify and report only evidence-backed conclusions.";
            } else {
                $benchmarkFallback = "Known: the request intent is understood but a reliable model inference was not completed in time.\n"
                    . "Unknown: final validated details that require model execution output.\n"
                    . "Next checks:\n"
                    . "1. Retry the same prompt with a larger timeout budget.\n"
                    . "2. Provide the exact artifact/data needed for deterministic verification.\n"
                    . "3. Re-run to confirm the result against retrieved evidence.";
            }
        }
...
$verificationContext = [
    ...
    'web_search_capability' => [
        'enabled' => (bool)($osRuntimeDecision['research_required'] ?? false),
...
if (!($verificationSummary['passed'] ?? false)) {
    ...
    $answerabilityMode = strtoupper(trim((string)($answerabilityState['mode'] ?? '')));
        $regenEligibleClasses = [
        'UNNECESSARY_EVIDENCE_REFUSAL',
        'FALSE_PREMISE_FAILURE',
        'QUANTITATIVE_ERROR',
        'LOGIC_ERROR',
        'WRITING_INSTRUCTION_FAILURE',
        'CASUAL_TONE_FAILURE',
        'SECURITY_FACT_ERROR',
        ...
        || ($benchmarkMode && $validationFailureClass === 'MISSING_EXTERNAL_EVIDENCE');
        $allowValidationRegeneration = !$streamUltraFastMode
        && $regenAllowedByAnswerability
        && in_array($validationFailureClass, $regenEligibleClasses, true)
        && !in_array($answerabilityMode, $blockedModes, true);
        ...
        if ($allowValidationRegeneration) {
                ...
                $regenIntent = in_array($validationFailureClass, ['QUANTITATIVE_ERROR', 'LOGIC_ERROR', 'FALSE_PREMISE_FAILURE', 'PRODUCTION_SAFETY_FAILURE', 'MISSING_EXTERNAL_EVIDENCE'], true)
            ? 'reasoning'
...
$allowEscalation = !$benchmarkMode && !$streamUltraFastMode && api_get_secret('CHAT_DYNAMIC_ESCALATION', '1') === '1';
...
$toolStateSnapshot = is_array($verificationSummary['tool_state'] ?? null)
    ? $verificationSummary['tool_state']
    : (is_array($webSearchLifecycle ?? null) ? $webSearchLifecycle : []);
$selectedTools = [];
if (($toolStateSnapshot['tool_required'] ?? false) && trim((string)($toolStateSnapshot['tool_name'] ?? '')) !== '') {
    $selectedTools[] = (string)$toolStateSnapshot['tool_name'];
}
$executionRecords = is_array($toolStateSnapshot['execution_records'] ?? null) ? $toolStateSnapshot['execution_records'] : [];
$executionId = '';
if (($toolStateSnapshot['tool_required'] ?? false) && !empty($executionRecords) && is_array($executionRecords[0] ?? null)) {
    $executionId = (string)($executionRecords[0]['execution_id'] ?? '');
}
$verificationState = ((bool)($verificationSummary['passed'] ?? false) && (($toolStateSnapshot['tool_required'] ?? false) === false || !empty($toolStateSnapshot['tool_result_verified'])))
    ? 'VERIFIED'
    : 'UNVERIFIED';
$validationState = is_string($validationResult ?? null) && trim((string)$validationResult) !== ''
...
$forensicPost = [
    ...
    'risk_level' => (string)($osRuntimeDecision['risk_level'] ?? 'LOW'),
    'routing_class' => (string)($osRuntimeDecision['route_class'] ?? 'STANDARD'),
    'selected_model' => (string)($mainMeta['model'] ?? $userModel),
    'selected_tools' => $selectedTools,
    'execution_id' => $executionId,
    'verification_state' => $verificationState,
    'validation_result' => $validationState,
];
trace_add($trace, $liveTrace, 'forensic', 'AI-OS forensic post-validation snapshot', $forensicPost);
