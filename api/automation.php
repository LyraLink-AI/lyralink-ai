...
function automation_execute(mysqli $db, array $auto, string $idempotencyKey = ''): array {
    ...
    $automationTask = chat_os_cron_job_context(
        'automation_execute',
        'Execute automation "' . trim((string)($auto['name'] ?? '')) . '": ' . $prompt,
        [
            'db' => $db,
            'risk_level' => 'MEDIUM',
            'task_domain' => 'automation',
            'user_id' => (string)$uid,
            'session_id' => 'automation:' . $autoId,
    ...
    $automationTask = is_array($automationTask['task'] ?? null) ? $automationTask['task'] : [];
    ...
    try {
        ...
        if ($automationTask !== []) {
            $automationTask = chat_os_mark_task_result($automationTask, 'SUCCEEDED', ['automation_id' => $autoId, 'result_preview' => mb_substr($reply, 0, 512)], null, null);
            chat_os_persist_task_record($automationTask, ['job_name' => 'automation_execute', 'automation_id' => $autoId], $db);
        }
        ...
        if ($automationTask !== []) {
            $automationTask = chat_os_mark_task_result($automationTask, 'FAILED', ['automation_id' => $autoId, 'error' => $errMsg], 'AUTOMATION_EXECUTION_FAILED', $errMsg);
            chat_os_persist_task_record($automationTask, ['job_name' => 'automation_execute', 'automation_id' => $autoId], $db);
        }
        $stmt = $db->prepare(
